...
class cifar_dataset(Dataset): 
    ...
    def get_noisy_data(self, cls_num, noise_file, noise_mode, noise_ratio):
        ...
        if noise_mode == 'tabasco':
            print("TABASCO Data Split") 
            
            # noisy_transaction_matrix_real = flip_labels_c_1(noise_ratio, self.cls_num)
            noisy_transaction_matrix_real = uniform_mix_c_1(noise_ratio, self.cls_num)

            noise_label = copy.deepcopy(train_label)
            for i in range(sum(self.img_num_list) ):
                noise_label[i] = np.random.choice(self.cls_num, p=noisy_transaction_matrix_real[train_label[i]])
    ...
    def get_noisy_imbalanced_per_cls_list(self, nlt_list):
        NLT_list = np.zeros(self.cls_num)
        for i in nlt_list:
            NLT_list[i] += 1
...
class cifar_dataloader():  
    def __init__(self, dataset, imb_type, imb_factor, noise_mode, noise_ratio, batch_size, num_workers, root_dir, log, noise_file=''):
        ...
        if self.dataset=='cifar10':
            ...
            self.transform_eval_train = transforms.Compose([
                    transforms.ToTensor(),
                    transforms.Normalize((0.4914, 0.4822, 0.4465),(0.2023, 0.1994, 0.2010)),
                ])
