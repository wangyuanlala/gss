...
class SupConResNet(nn.Module):
    ...
    def __init__(self, num_classes, head='mlp', feat_dim=32, ckpt=None):
        ...
        self.encoder.load_state_dict(dict(ckpt[0]))
        self.encoder = nn.Sequential(*list(self.encoder.children())[:-1])
        self.classifier = nn.Linear(dim_in, num_classes)

    def forward(self, x):
        feat = self.encoder(x)
        feat = F.adaptive_avg_pool2d(feat, (1, 1)).view(-1, 64)
        outpus = self.classifier(feat)
        return feat, outpus
